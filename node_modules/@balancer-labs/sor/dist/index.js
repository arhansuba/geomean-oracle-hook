...
const normaliseBalance = (token) => {
    return bignumber.parseFixed(token.balance, 18)
        .mul(bignumber.parseFixed(token.priceRate ?? '1', 18))
...
const normaliseAmount = (amount, token) => {
    ...
    return bignumber.BigNumber.from(_upscale(amount, scalingFactor).toString())
        .mul(bignumber.parseFixed(token.priceRate ?? '1', 18))
        .div(constants.WeiPerEther)
        .toBigInt();
};
...
const denormaliseAmount = (amount, token) => {
    const amountAfterRate = (amount * ONE) /
        BigInt(bignumber.parseFixed(token.priceRate ?? '1', 18).toString());
    const scalingFactor = _computeScalingFactor(BigInt(token.decimals));
...
class WeightedPool {
    ...
    _exactTokenInForTokenOut(poolPairData, amount) {
        ...
        const balanceIn = bignumber.parseFixed(poolPairData.balanceIn.toString(), 18 - decimalsIn).toBigInt();
        const balanceOut = bignumber.parseFixed(poolPairData.balanceOut.toString(), 18 - decimalsOut).toBigInt();
    ...
    _calcTokensOutGivenExactBptIn(bptAmountIn) {
        ...
        try {
            ...
            return amountsOut.map((a) => bignumber.BigNumber.from(a));
        }
        catch (err) {
...
function _invariant$1(A, balances // balances
    ...
    const AAdjusted = bnum(bignumber.formatFixed(A, 3));
...
function _solveAnalyticalBalance$1(sum, inv, A, n_pow_n, p) {
    // A is passed as an ethers bignumber while maths uses bignumber.js
    const oldBN_A = bnum(bignumber.formatFixed(A, 3));
    //Round up p
...
function _spotPriceAfterSwapExactTokenInForTokenOut$4(amount, poolPairData) {
    ...
    ans = ONE$1.div(ans.times(constants.WeiPerEther.sub(swapFee).toString()).div(constants.WeiPerEther.toString()));
    return ans;
}
// PairType = 'token->token'
// SwapType = 'swapExactOut'
...
function _spotPriceAfterSwapTokenInForExactBPTOut$2(amount, poolPairData) {
    ...
    let ans = _poolDerivativesBPT$2(A, balances, bnum(bignumber.formatFixed(balanceOut, decimalsOut)).plus(amount), tokenIndexIn, true, true, true);
...
class MetaStablePool {
    ...
    _calcTokensOutGivenExactBptIn(bptAmountIn) {
        ...
        try {
            const amountsOutNormalised = _calcTokensOutGivenExactBptIn$1(balancesNormalised, bptAmountIn.toBigInt(), this.totalShares.toBigInt());
            const amountsOut = amountsOutNormalised.map((a, i) => bignumber.BigNumber.from(denormaliseAmount(a, this.tokens[i]).toString()));
            return amountsOut;
        }
        catch (err) {
            return new Array(balancesNormalised.length).fill(ZERO);
    ...
    _spotPriceAfterSwapExactTokenInForTokenOut(poolPairData, amount) {
        const priceRateIn = bnum(bignumber.formatFixed(poolPairData.tokenInPriceRate, 18));
        const priceRateOut = bnum(bignumber.formatFixed(poolPairData.tokenOutPriceRate, 18));
...
class LinearPool {
    ...
    updateTokenBalanceForPool(token, newBalance) {
        ...
        if (isSameAddress(this.address, token)) {
            const parsedTokenBalance = bignumber.parseFixed(T.balance, T.decimals);
...
class PhantomStablePool {
    ...
    _spotPriceAfterSwapExactTokenInForTokenOut(poolPairData, amount) {
        const priceRateIn = bnum(bignumber.formatFixed(poolPairData.tokenInPriceRate, 18));
        const priceRateOut = bnum(bignumber.formatFixed(poolPairData.tokenOutPriceRate, 18));
        const amountConverted = amount.times(bnum(bignumber.formatFixed(poolPairData.tokenInPriceRate, 18)));
        let result;
        if (poolPairData.pairType === PairTypes.TokenToBpt) {
...
function normalizeBalances$2(balances, decimals, tokenRates) {
    const scalingFactors = decimals.map((d) => bignumber.parseFixed('1', d));
    return balances.map((bal, index) => mulDown(bal.mul(constants.WeiPerEther).div(scalingFactors[index]), tokenRates[index]));
}
...
const parseFixedCurveParam = (param) => {
    const param64 = ((((BigInt(bignumber.parseFixed(param, 18).toString()) + 1n) << 64n) /
        10n ** 18n) *
        10n ** 36n) >>
        64n;
...
// Constants
const ONE_36 = bignumber.parseFixed('1', 36);
const ONE_18 = bignumber.parseFixed('1', 18);
const OLD_ONE_36 = bnum(10).pow(36);
const CURVEMATH_MAX_DIFF_36 = bignumber.parseFixed('-0.000001000000000000024', 36);
const ONE_TO_THE_THIRTEEN_NUM_36 = bignumber.parseFixed('10000000000000', 36);
const CURVEMATH_MAX_36 = bignumber.parseFixed('0.25', 36); //CURVEMATH MAX from contract
var CurveMathRevert;
...
const getParsedFxPoolData = (amount_36, poolPairData, isOriginSwap) => {
    ...
    return {
        ...
        beta_36: bignumber.parseFixed(poolPairData.beta.toString(), 18),
        delta_36: bignumber.parseFixed(poolPairData.delta.toString(), 18),
        epsilon_36: bignumber.parseFixed(poolPairData.epsilon.toString(), 18),
        lambda_36: bignumber.parseFixed(poolPairData.lambda.toString(), 18),
...
const calculateMicroFee = (_bal, _ideal, _beta, _delta) => {
    ...
    let fee_ = bignumber.BigNumber.from(0);
    if (_bal.lt(_ideal)) {
        ...
        if (_bal.lt(_threshold)) {
            ...
            fee_ = bignumber.BigNumber.from(0);
        }
    }
    else {
...
const calculateFee = (_gLiq, _bals, _beta, _delta, _weights) => {
    ...
    let psi_36 = bignumber.BigNumber.from(0);
...
const spotPriceBeforeSwap = (amount_36, poolPairData) => {
    ...
    bignumber.parseFixed('1', 36), // input amount
...
const _derivativeSpotPriceAfterSwapExactTokenInForTokenOut = (amount, poolPairData) => {
    const x = spotPriceBeforeSwap(bignumber.parseFixed('1', 36), poolPairData);
...
class FxPool {
    constructor(id, address, swapFee, totalShares, tokens, tokensList, alpha, beta, lambda, delta, epsilon) {
        this.poolType = exports.PoolTypes.Fx;
        this.id = id;
        this.address = address;
        this.swapFee = bignumber.parseFixed(swapFee, 18);
        this.totalShares = bignumber.parseFixed(totalShares, 18);
        ...
        this.delta = bnum(bignumber.parseFixed(delta, 18).toString());
    ...
    parsePoolPairData(tokenIn, tokenOut) {
        ...
        const poolPairData = {
            ...
            tokenInLatestFXPrice: bignumber.parseFixed(tI.token.latestFXPrice, tI.token.fxOracleDecimals),
            tokenOutLatestFXPrice: bignumber.parseFixed(tO.token.latestFXPrice, tO.token.fxOracleDecimals),
...
function parseNewPool(pool, currentBlockTimestamp = 0) {
    ...
    try {
        ...
        if (!isLinear && !(pool.poolType in exports.PoolFilter)) {
...
function getBoostedGraph(tokenIn, tokenOut, poolsAllDict, config) {
    ...
    for (const id in poolsAllDict) {
        ...
        if (pool.poolType == exports.PoolTypes.Linear) {
            ...
            else if (pool.poolType === exports.PoolTypes.Weighted) {
...
class SwapCostCalculator {
     ...
     */
    async convertGasCostToToken(tokenAddress, tokenDecimals, gasPriceWei, swapGas = bignumber.BigNumber.from('85000')) {
...
class SOR {
    ...
    async getSwaps(tokenIn, tokenOut, swapType, swapAmount, swapOptions, useBpts = false) {
        ...
        const wrappedInfo = await getWrappedInfo(this.provider, swapType, tokenIn, tokenOut, this.config, bignumber.BigNumber.from(swapAmount));
        let swapInfo;
    ...
    async processSwaps(tokenIn, tokenOut, swapType, swapAmount, pools, swapOptions) {
        ...
        const costOutputToken = await this.getCostOfSwapInToken(swapType === exports.SwapTypes.SwapExactIn ? tokenOut : tokenIn, swapType === exports.SwapTypes.SwapExactIn
            ? tokenOutDecimals
...
bptTotalSupply, amp) {
    ...
    const amountBPTOut = amounts.reduce((totalBptOut, amountIn, i) => {
        // Calculate amount of BPT gained per token in
        const poolPairData = {
            amp: bignumber.BigNumber.from(amp),
...
exports.FxMaths = fxPoolMath;
exports.FxPool = FxPool;
exports.Gyro2Maths = gyro2Math;
exports.Gyro2Pool = Gyro2Pool;
exports.Gyro2V2Pool = Gyro2V2Pool;
exports.Gyro3Maths = gyro3Math;
exports.Gyro3Pool = Gyro3Pool;
exports.GyroEMathFunctions = gyroEMathFunctions;
exports.GyroEMaths = gyroEMath;
exports.GyroEV2Pool = GyroEV2Pool;
exports.GyroHelpers = helpers;
exports.GyroHelpersSignedFixedPoint = gyroSignedFixedPoint;
exports.LinearMaths = linearMath;
...
exports.ZERO = ZERO;
exports.balancesFromTokenInOut = balancesFromTokenInOut;
exports.bnum = bnum;
...
exports.safeParseFixed = safeParseFixed;
